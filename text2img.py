...
def text2img(text, font, img_path):
    ...
    # centered_img = centered_img.resize((64, 64), Image.ANTIALIAS).convert('L')
    # centered_img = centered_img.resize((64, 64), Image.ANTIALIAS)
...
if generate_all:
    ...
    min_h = 9999
    min_w = 9999
    with open("showa_char.txt", "r") as ins:
        jchars = []
        for line in ins:
            jchars.append(line.rstrip().split()[0])
        print(jchars)
    # for i in range(0x3040, 0x309F):
    for text in jchars:
        # text = chr(i)
